...
function ToolTips({ text, onClick }: ToolTipsProps) {
  return (
    <button className="relative bg-green-400 rounded-full p-4 hover:shadow-md" onClick={onClick}>
      ...
      <div className="absolute top-[-80%] translate-x-[-100%] bg-gray-200 p-1 peer-hover:opacity-100 opacity-0 before:content-['▼'] before:absolute before:bottom-[-40%] before:left-[50%] before:text-gray-200">
        {text}
      </div>
    </button>
  );
